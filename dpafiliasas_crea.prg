...
PROCE MAIN()
  ...
  IF !EJECUTAR("DBISTABLE",oDp:cDsnConfig,"DPAFILIASAS",.F.)
     Checktable("DPAFILIASAS")
  ENDIF
  ...
  IF !ISSQLFIND("DPAFILIASAS","SAS_ID"+GetWhere("=",oDp:cCodSas))
      ...
      EJECUTAR("CREATERECORD","DPAFILIASAS",{"SAS_ID"    ,"SAS_ACTIVO"},;
                                            {oDp:cCodSas ,.T.         },;
                                            NIL,.T.,"SAS_ID"+GetWhere("=",oDp:cCodSas))
  ENDIF
